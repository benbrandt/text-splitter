...
use text_splitter::{Characters, ChunkCapacity, ChunkSize, ChunkSizer};
...
enum TextSplitterOptions {
    Characters(text_splitter::TextSplitter<Characters>),
    CustomCallback(text_splitter::TextSplitter<CustomCallback>),
    Huggingface(text_splitter::TextSplitter<Tokenizer>),
    Tiktoken(text_splitter::TextSplitter<CoreBPE>),
}
    ...
    fn new(trim_chunks: bool) -> Self {
        Self {
            splitter: TextSplitterOptions::Characters(
                text_splitter::TextSplitter::default().with_trim_chunks(trim_chunks),
            ),
        }
    }
    ...
    fn from_huggingface_tokenizer(tokenizer: &PyAny, trim_chunks: bool) -> PyResult<Self> {
        ...
        Ok(Self {
            splitter: TextSplitterOptions::Huggingface(
                text_splitter::TextSplitter::new(tokenizer).with_trim_chunks(trim_chunks),
            ),
        })
    }
    ...
    fn from_callback(callback: PyObject, trim_chunks: bool) -> Self {
        Self {
            splitter: TextSplitterOptions::CustomCallback(
                text_splitter::TextSplitter::new(CustomCallback(callback))
                    .with_trim_chunks(trim_chunks),
            ),
        }
    }
